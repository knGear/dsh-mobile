...
`apk/src/main/AndroidManifest.xml`：`versionCode`（递增）+ `versionName`（语义版本）。
...
- 壳改动：开发目录 `apk/src/main/` → 仓库 `apk/src/main/`（**manifest 版本号不同步**，仓库保持发布版）
